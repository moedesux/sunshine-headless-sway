...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force add games even if they already exist in Sunshine (overwrites existing entries).",
    )
    parser.add_argument(
        "--sunshine-host",
        default="",
...
def main(argv=None):
    ...
    try:
        ...
        existing_apps_full, existing_error = _get_full_sunshine_apps(allow_prompt=False)
        existing_game_names = {app["name"] for app in existing_apps_full}
        existing_app_by_name = {app["name"]: app.get("index", -1) for app in existing_apps_full}
        if existing_error:
            print(f"Warning: Could not retrieve existing apps for duplicate check: {existing_error}")
            existing_game_names = set()
            existing_app_by_name = {}

        all_games.sort(key=lambda x: x[1])

        # Hardcoded Steam Big Picture entry (not in Steam library)
        all_games.insert(0, ("bigpicture", "Steam Big Picture", "Steam", "Steam"))

        for idx, (_, game_name, display_source, source) in enumerate(all_games):
            if game_name in existing_game_names:
                status = f"(already in {get_server_display_name()})" + (" [will be overwritten]" if args.force else "")
            else:
                status = ""
            if len(futures) > 1: 
        ...
        selected_games = [all_games[i] for i in selected_indices if all_games[i][1] not in existing_game_names or args.force]

        if not selected_games:
            if args.force:
                print(f"No games selected. Use --all --force to force-add all games, or select specific games by number.")
            else:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for game_id, game_name, display_source, source in valid_selected_games:
                if download_images and api_key:
                    app_index = existing_app_by_name.get(game_name) if args.force else None
                    future = executor.submit(download_image_from_steamgriddb, game_name, api_key)
                    futures[future] = (game_id, game_name, source, app_index)
                else:
                    app_index = existing_app_by_name.get(game_name) if args.force else None
                    add_game_to_sunshine(game_id, game_name, DEFAULT_IMAGE, source, index=app_index)
                    games_added = True

            for future in as_completed(futures):
                game_id, game_name, source, app_index = futures[future]
                try:
                ...
                add_game_to_sunshine(game_id, game_name, image_path, source, index=app_index)
                games_added = True

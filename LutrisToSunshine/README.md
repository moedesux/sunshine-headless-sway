...
LutrisToSunshine imports games from supported launchers into **Sunshine or Apollo** and can optionally download cover art from SteamGridDB. It also includes a guided virtual-display mode for headless streaming, so streamed games can run on a separate desktop without disturbing your main session.
...
- Import games from multiple launchers into Sunshine or Apollo from one CLI
...
- Heroic: native and Flatpak (Legendary, GOG, Nile, and Sideload runners)
- Bottles: Flatpak
...
All host and port settings can also be controlled via environment variables:

- `LUTRISTOSUNSHINE_SUNSHINE_HOST` or `LUTRISTOSUNSHINE_API_HOST`: override the web UI host
- `LUTRISTOSUNSHINE_SUNSHINE_PORT` or `LUTRISTOSUNSHINE_API_PORT`: override the web UI port

Server-specific overrides take precedence when both servers are detected:

- `LUTRISTOSUNSHINE_APOLLO_HOST` / `LUTRISTOSUNSHINE_APOLLO_PORT`: apply only when Apollo is active
...
3. Lists the games it can import (includes a "Steam Big Picture" entry for Steam users)
...
```bash
python3 lutristosunshine.py display mangohud-fps-limit disable
```

Set dynamic refresh rate sync mode: client, exact, or custom:

```bash
python3 lutristosunshine.py display refresh-rate-mode client
python3 lutristosunshine.py display refresh-rate-mode exact
python3 lutristosunshine.py display refresh-rate-mode custom --width 1920 --height 1080 --refresh 60
```
